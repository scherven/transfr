...
```bash
bash core/dbgen/extract_europe.sh
...
`core/dbgen/schema.sql` creates the four raw tables, all query indexes (including the
...
PGDATABASE=transfr_eu .venv/bin/python core/dbgen/build_station_index.py     # station_points (centroids)
PGDATABASE=transfr_eu .venv/bin/python core/dbgen/build_platform_index.py    # station_stops + osm_nodes coord index (Tier 2)
...
# 2b. Create DB + schema
createdb transfr_kr
psql -d transfr_kr -v ON_ERROR_STOP=1 -f core/dbgen/schema.sql

# 2c. Load
PGDATABASE=transfr_kr .venv/bin/python core/dbgen/etl.py core/data/south-korea-railway-pedestrian.pbf

# 2d. Derived tables (same order as Europe)
PGDATABASE=transfr_kr .venv/bin/python core/dbgen/build_node_way_ids.py
PGDATABASE=transfr_kr .venv/bin/python core/dbgen/build_station_index.py
PGDATABASE=transfr_kr .venv/bin/python core/dbgen/build_platform_index.py
PGDATABASE=transfr_kr .venv/bin/python core/dbgen/build_stitch_bridges.py
...
```bash
PGDATABASE=transfr_eu .venv/bin/python core/dbgen/build_node_way_ids.py            # always TRUNCATE+rebuild
PGDATABASE=transfr_eu .venv/bin/python core/dbgen/build_station_index.py --rebuild
PGDATABASE=transfr_eu .venv/bin/python core/dbgen/build_platform_index.py --rebuild
PGDATABASE=transfr_eu .venv/bin/python core/dbgen/build_stitch_bridges.py --rebuild
```
...
```bash
PGDATABASE=transfr_eu .venv/bin/python core/dbgen/build_stitch_bridges.py \
