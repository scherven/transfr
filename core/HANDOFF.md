...
**Extraction** (`core/extract_europe.sh`): two `osmium` passes, cheapest
reduction first —
...
- `node_way_ids(node_id PK, way_ids bigint[])` — added later, see
  ...
  you rebuild from scratch, `core/build_node_way_ids.py` must be run after
  `core/etl.py`.

Loading: `core/etl.py`, pyosmium-based, batched commits (nodes every 50k,
...
progress). Full load took ~55 minutes. `core/build_node_way_ids.py` (the
...
1. **A\* search** (`core/algo_astar.py`) — heuristic
...
2. **`node_way_ids` materialized adjacency table**
   (`core/build_node_way_ids.py`, wired into `SearchContext.expand()` via
...
Both are wired through a pluggable registry (`core/algorithms.py`,
...
up in `core/benchmark.py`'s comparison table — no other test-file changes
needed.
...
Files: `core/bidirectional_search.py` (implementation),
...
**Next steps, in order**:
1. Rewrite `bidirectional_shortest_path()` in `core/bidirectional_search.py`
...
- `core/generate_verification_report.py` regenerates the hand-verification
