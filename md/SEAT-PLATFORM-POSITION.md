...
|---|---|---|---|
| [`core/boarding/seat.py`](boarding.py) | algorithm | `PlatformGeometry`, `TrainFormation`, seat→point (`resolve_alighting_point`), graph seeding (`insert_start_point`, `boarding_source_distances`), `find_path_from_seat`. Pure, DB-independent. | [`test_boarding.py`](../tests/test_boarding.py) — 21 |
| [`core/boarding/formation_model.py`](formation_model.py) | data | Normalizes any operator's data into `NormalizedFormation`; resolution ladder metres→sector→order; `PlatformSectorMap` (incl. OSM section-sign constructor); `to_train_formation()` bridge. | covered via providers |
| [`core/boarding/formation_providers.py`](formation_providers.py) | data | Per-operator adapters (DE/CH/AT/NL/FR/GB) + capability-gap entries (IT/ES/JP); `promise` scoring; `rank_providers()`. Schemas *representative*. | [`test_formation_providers.py`](../tests/test_formation_providers.py) — 47 |
| [`core/boarding/live_sources.py`](live_sources.py) | data | **Live** fetch from real APIs + real-schema parsers: `fetch_db_departures`, `fetch_transitous_plan`, `parse_wagenreihung`, `fetch_db_formation`, `sector_map_from_wagenreihung`. | [`test_live_sources.py`](../tests/test_live_sources.py) — 8 offline + 3 live (gated) |
| [`core/tooling/pull_live_demo.py`](pull_live_demo.py) | data | Runnable end-to-end demo on real data. | — |
...
- `api.transitous.org/api/v5/plan` — MOTIS 2, real per-stop platform/track +
  coordinates, no key. (Same API `api/journeys.py` already uses.)
...
.venv/bin/python core/tooling/pull_live_demo.py
```
